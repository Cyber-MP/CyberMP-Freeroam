...
import { r } from '@/rpc';
import { pvpContract } from './-contract';

export const Route = createFileRoute('/hud/game-modes/pvp/')({
...
const ReleaseCountdown = () => {
  ...
  useEffect(() => {
    ...
    r.implement(pvpContract.setCountdownText, handler);

    return () => {
      r.unimplement(pvpContract.setCountdownText);
      if (clearTimeoutId) {
        clearTimeout(clearTimeoutId);
      }
    };
...
const DrawTimer = () => {
  ...
  useImplement(pvpContract.startDrawTimer, (c) => {
