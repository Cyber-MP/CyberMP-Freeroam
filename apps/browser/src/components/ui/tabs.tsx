...
const tabsListVariants = cva(
  ...
  'group/tabs-list inline-flex w-fit items-center justify-center rounded-md p-1.5 text-muted-foreground group-data-horizontal/tabs:h-12 group-data-vertical/tabs:h-fit group-data-vertical/tabs:flex-col data-[variant=line]:rounded-none',
  {
    variants: {
      variant: {
        default: 'bg-muted',
  ...
  return (
    <TabsPrimitive.Trigger
      ...
      className={cn(
        ...
        'relative inline-flex h-full flex-1 items-center justify-center gap-2 rounded-sm border border-transparent px-4 py-2 text-base font-medium切换 transition-all',
...
function TabsContent({
  ...
  return (
    <TabsPrimitive.Content
      ...
      className={cn('flex-1 text-sm/relaxed outline-none mt-4', className)}
      {...props}
    />
  );
}
